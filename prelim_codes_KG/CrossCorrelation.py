...
def CrossCorr2pt(bins, boxsize, pos, delta, thickness, threads=8):
    '''
    Parameters
    ----------
...
def tracer_field_cross_CDF(data_pos, bins, delta_matter, matter_grid, query_type, query_grid, delta_threshold, boxsize, kNN, n_threads):
    '''
...
def Sampling2ptCC(bins, boxsize, pos, delta, thickness, n_gal, n_data, n_null):
    '''     
    Parameters
    ----------
    bins : float array of shape (m,)
    ...
    xi : float array of shape (n_data, bin_length)
        The 2-point cross-correlation function (2PCF) between the galaxy positions and the field, computed at each radial bin.
    ...
    for i in range(n_data):
        np.random.seed()  # ensure a new seed each iteration
...
def Sampling_kNN_CC(bins, boxsize, pos, delta, matter_grid, query_type, query_grid, threshold, kNN, n_gal, n_data, n_null, n_threads=8):
    ''' 
    Parameters
    ----------
