...
#true_noise_seed = 1757 #52520 #53827#68716 #76231 #537691
...
psi_fid, bins_1nn, n_gal  = process_reion_case("fiducial",z,omega_m, omega_b, h, true_noise_seed, n_null) #psi_fid.shape = (n_data, bin_len)

psi_low, _ , _   = process_reion_case("alt_case_low",z,omega_m, omega_b, h, true_noise_seed, n_null)

psi_high, _ , _  = process_reion_case("alt_case_high",z,omega_m, omega_b, h, true_noise_seed, n_null)
...
data_low_1nn = np.mean(psi_low, axis=0)
are_equal = np.array_equal(psi_low[0], data_low_1nn)
print(f"Exact equality ?: {are_equal}")
print("data_low_1nn = ",data_low_1nn)

data_high_1nn = np.mean(psi_high, axis=0)
are_equal = np.array_equal(psi_high[0], data_high_1nn)
print(f"Exact equality ?: {are_equal}")
print("data_high_1nn = ",data_high_1nn)



chi_sq_1nn_fid_low = (np.transpose(data_fid_1nn - data_low_1nn)).dot(c_inv_1nn).dot(data_fid_1nn - data_low_1nn)
chi_sq_1nn_fid_high = (np.transpose(data_fid_1nn - data_high_1nn)).dot(c_inv_1nn).dot(data_fid_1nn - data_high_1nn)
chi_sq_1nn_low_high = (np.transpose(data_low_1nn - data_high_1nn)).dot(c_inv_1nn).dot(data_low_1nn - data_high_1nn)
plt.figure(figsize=(10,8))
plt.plot(bins_1nn, data_fid_1nn, color='C0', ls='solid', label=r'Fiducial ($\alpha_{\mathrm{esc}} = -0.3$)', lw=2)
plt.plot(bins_1nn, data_low_1nn, color='C2', ls='dashed', label=r"$\alpha_{\mathrm{esc}} = -0.45 : \chi^2_{\rm 1NN;(fid,M1)} =$"+str(np.round(chi_sq_1nn_fid_low,3)), lw=2)
plt.plot(bins_1nn, data_high_1nn, color='C1', ls='dashed', label=r"$\alpha_{\mathrm{esc}} = -0.15 : \chi^2_{\rm 1NN;(fid,M2)} =$"+str(np.round(chi_sq_1nn_fid_high,3)), lw=2)
for psi in psi_null:
    plt.plot(bins_1nn, psi, color='black', alpha=0.05, lw=0.5)

plt.plot(bins_1nn, null_1nn, color='black', ls='solid', label="Random galaxies x (21cm Noise + Wedge) \n"+r"($N_{\mathrm{null,real}} = $"+str(n_null)+")", lw=2)
plt.xlabel('r (cMpc/h)')
plt.ylabel(r'$\psi_{\mathrm{1NN}}(r)$')
plt.title('1NN-CDF cross-correlation \n '+str(n_gal)+' galaxies per set , SKA-AA* ('+str(tobs)+'hrs) + Foreground Wedge ($m_w$ = '+str(slope_wedge)+')',fontsize=14)

plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True, color='gray', linestyle='--', linewidth=0.5, alpha=0.7)
plt.savefig(os.path.join(outdir,f"1NN_stat_comparision_nreal{n_null}_region{region}.pdf"), dpi=300)



# Print results 
print("===================================")
print("[SUB-BOX "+str(region)+ " ] Chi-square comparison results:")
print("-----------------------------------")
print(f"χ²(fiducial vs low) = {chi_sq_1nn_fid_low:.4f}")
print(f"χ²(fiducial vs high) = {chi_sq_1nn_fid_high:.4f}")
print(f"χ²(low vs high) = {chi_sq_1nn_low_high:.4f}")
print("===================================")
...
plt.savefig(os.path.join(outdir,f"1NN_corrMat_comparision_nreal{n_null}_region{region}.pdf"), dpi=300)
